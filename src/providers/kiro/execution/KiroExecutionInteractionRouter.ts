...
interface KiroQuestion {
  readonly id?: string;
  readonly multiSelect: boolean;
  readonly options: readonly KiroQuestionOption[];
  readonly question: string;
}

export class KiroExecutionInteractionRouter {
  private sequence = 0;
  ...
  async handle(method: string, params: unknown, signal?: AbortSignal): Promise<unknown> {
    ...
    throw new Error(`Unsupported Kiro server request: ${method}`);
  }
...
): { questions: KiroQuestion[]; sessionId: string; toolCallId: string } | null {
...
function buildQuestionResponse(
  questions: readonly KiroQuestion[],
