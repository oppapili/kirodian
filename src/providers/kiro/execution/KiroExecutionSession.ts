...
} from '../history/KiroHistoryPathResolver';
import {
  decodeKiroModelId,
  findKiroModel,
...
import { waitForKiroCancelDelivery } from '../runtime/KiroCancelDelivery';
...
import { KiroExecutionInteractionRouter } from './KiroExecutionInteractionRouter';
import {
  normalizeKiroModelUpdateMetadata,
...
class KiroExecutionRunState implements ProviderExecutionRun {
...
export class KiroExecutionSession
implements
ProviderExecutionSession,
SteerableExecutionSession,
ModeConfigurableExecutionSession,
RewindableExecutionSession {
  ...
  constructor(
    ...
    private readonly options: KiroExecutionSessionOptions,
  ) {
    this.providerSessionId = config.resumeSeed?.providerSessionId;
    const providerState = parseKiroProviderState(config.resumeSeed?.providerState);
    ...
    this.interactionRouter = new KiroExecutionInteractionRouter(
  ...
  execute(request: ProviderExecutionRequest): ProviderExecutionRun {
    if (this.disposed) throw new Error('Kiro execution session is disposed.');
    if (this.active) throw new Error('Kiro execution session is already executing.');
    const run = new KiroExecutionRunState(
      randomUUID(),
      randomUUID(),
    ...
    const active: ActiveExecution = {
      ...
      normalizer: new AcpExecutionEventNormalizer({
        ...
        toolStreamAdapter: createKiroToolStreamAdapter(),
      }),
      request,
      run,
      sequence: 0,
  ...
  private async performExecution(active: ActiveExecution): Promise<void> {
    if (active.request.toolPolicy.kind === 'allow-list') {
      this.updateSnapshot('invalidated', {
        message: 'Exact Kiro tool allow-list enforcement is unavailable.',
      ...
      active.run.finish({
        ...
        message: 'Kiro does not support reliable exact allow-list enforcement.',
        recoverable: false,
  ...
  ): Promise<KiroExecutionNativeConnection> {
  ...
  private async startNative(
    ...
    const owner: KiroNativeOwner = {
      generation,
      initialized: false,
      loadedSessionConfigurationKey: null,
      loadedSessionId: null,
      modeUnsubscribe: () => {},
      modelContextKey: computeKiroEnvironmentHash(this.plugin.settings),
      modelsUnsubscribe: () => {},
      native,
  ...
  private async ensureSession(
    native: KiroExecutionNativeConnection,
    ...
    const state = parseKiroProviderState(this.providerState);
    if (state.forkSource && !this.forkApplied) {
      if (!native.fork || !state.forkSourceSessionDirectory) {
        throw new Error('Kiro fork metadata is incomplete.');
      }
      ...
      if (targetPromptIndex === null || targetPromptIndex === undefined) {
        throw new Error('The Kiro fork checkpoint could not be located.');
      }
      ...
      this.throwIfCancellationRequested(active);
      if (this.disposed) throw new KiroExecutionCancellationError();
      return this.loadProviderSession(
        native,
        sessionId,
        request,
  ...
  private getSystemPromptSettings(): KiroSystemPromptSettings {
    return {
  ...
  private async applyConfiguration(
    ...
    const rawModel = request.configuration.model
      ? decodeKiroModelId(request.configuration.model)
      : null;
    if (rawModel) {
      ...
      this.throwIfCancellationRequested(active);
      const model = normalizeKiroSetModelMetadata(rawModel, response._meta);
      if (model) {
  ...
  private resolveReasoningEffort(
    ...
    const model = findKiroModel(settings.currentCatalog?.models ?? [], rawModelId);
    if (model?.reasoningMetadataResolved !== true) return null;
    const advertisedValues = getKiroAvailableReasoningEfforts(model)
  ...
  private async cancelRun(run: KiroExecutionRunState, reason: string): Promise<void> {
    const active = this.active;
    ...
    this.cancellationFlight = (async () => {
      ...
      try {
        await waitForKiroCancelDelivery(
        ...
        if (!this.disposed) {
          this.updateSnapshot('invalidated', {
            message: 'The cancelled Kiro process was quarantined and will be replaced.',
            reason: 'cancelled',
            recoverable: true,
          });
          this.emitCurrentSnapshot();
  ...
  private throwIfCancellationRequested(active: ActiveExecution | undefined): void {
    if (active && this.isCancellationRequested(active)) {
      throw new KiroExecutionCancellationError();
    }
  }
  ...
  private createForkSession(
    ...
    request: Parameters<NonNullable<KiroExecutionNativeConnection['fork']>>[0],
  ): Promise<string> {
    if (this.forkCreationFlight) return this.forkCreationFlight;
    const fork = native.fork?.bind(native);
    if (!fork) return Promise.reject(new Error('Kiro fork metadata is incomplete.'));

    const flight = (async () => {
      const response = await fork(request);
      if (!response.newSessionId.trim()) {
        throw new Error('Kiro returned a fork without a child session.');
      }
      this.adoptForkSession(response.newSessionId);
      if (response.parentSessionId !== sourceSessionId) {
        throw new Error('Kiro returned a fork for an unexpected parent session.');
      }
  ...
  private shutdownNativeOwner(owner: KiroNativeOwner): Promise<void> {
  ...
  private isCurrentNativeOwner(owner: KiroNativeOwner): boolean {
    return !this.disposed
  ...
  private getNativeOwner(
    ...
    if (!owner || owner.native !== native) {
      throw new Error('Kiro native connection ownership changed.');
    }
    return owner;
  }
  ...
  private async performRewind(
    ...
    if (!this.providerSessionId || !assistantMessageId || !state.sessionDirectory) {
      return { canRewind: false, error: 'Kiro rewind metadata is unavailable.' };
    }
    if (!this.options.resolvePromptIndex) {
      return { canRewind: false, error: 'Kiro prompt index resolution is unavailable.' };
    }
    ...
    if (promptIndex === null) {
      return { canRewind: false, error: 'The Kiro prompt could not be located.' };
    }
    const native = await this.ensureNative();
    await this.ensureSession(native, undefined);
    if (!native.rewind) return { canRewind: false, error: 'Kiro rewind is unavailable.' };
  ...
  private captureProviderSession(
    ...
    const sessionDirectory = resolveKiroSessionDirectory(
      ...
      this.config.vaultWorkingDirectory,
      {
        environment: buildKiroRuntimeEnv(this.plugin.settings, ''),
  ...
  private async publishSessionModels(
    ...
    const { currentModelId, models } = normalizeKiroSessionModelMetadata(response);
    if (models.length > 0) {
      await this.mergeModelMetadataBestEffort(
        models,
  ...
  private async publishModelsFromConfig(
    ...
    const models = normalizeKiroDiscoveredModels(flat.map(option => ({
  ...
  private emitCurrentSnapshot(): void {
    ...
    for (const listener of this.listeners) {
      try {
        ...
        // Session listeners cannot interfere with native Kiro lifecycle.
      }
    }
  }
...
class KiroExecutionCancellationError extends Error {
  constructor() {
    super('Kiro execution was cancelled.');
    this.name = 'KiroExecutionCancellationError';
  }
}
...
function buildSessionMeta(
  ...
  const rawModel = request.configuration.model
    ? decodeKiroModelId(request.configuration.model)
    : null;
  const systemPromptOverride = buildKiroSystemPromptOverride(
    request,
...
function buildKiroSystemPromptOverride(
  ...
  return [instructions, KIRO_PASSIVE_TOOL_INSTRUCTION]
    .filter(Boolean)
    .join('\n\n');
}
