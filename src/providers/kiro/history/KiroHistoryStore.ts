...
export interface KiroHistoryUsage {
  inputTokens?: number;
  outputTokens?: number;
...
export interface ParsedKiroHistory {
  lastUsage?: KiroHistoryUsage;
  messages: ChatMessage[];
}

interface KiroHistoryRecord {
  method: string;
...
interface CompletedTurn {
  messages: ChatMessage[];
  promptIndex: number;
  usage?: KiroHistoryUsage;
}

export function parseKiroHistoryContent(
  content: string,
  sessionId: string,
): ParsedKiroHistory {
  ...
  for (const line of content.split(/\r?\n/)) {
    ...
    if (updateType === 'user_message_chunk') {
      const incomingUserId = resolveKiroUpdateMessageId(
        update,
        'user',
        record.params._meta,
      );
    ...
    if (updateType === 'agent_message_chunk') {
      ...
      pending.assistantId ??= resolveKiroUpdateMessageId(
        update,
        'assistant',
        record.params._meta,
      );
  ...
  let lastUsage: KiroHistoryUsage | undefined;
...
export function resolveKiroPromptIndexAfterAssistant(
  ...
  for (const line of content.split(/\r?\n/)) {
    ...
    if (
      ...
      && resolveKiroUpdateMessageId(update, 'assistant', record.params._meta) === resumeAt
      && activePromptIndex !== null
    ) {
...
function resolveLegacyForkTargetPromptIndex(
  ...
  for (const message of parseKiroHistoryContent(content, sessionId).messages) {
...
export async function loadKiroPromptIndexAfterAssistant(
  sessionDirectory: string,
  sessionId: string,
  resumeAt: string,
): Promise<number | null> {
...
function reconcileToolUpdate(turn: PendingTurn, update: Record<string, unknown>): void {
  ...
  const rawNameResolution = resolveKiroRawToolName(current ? {
  ...
  const rawOutput = update.rawOutput !== undefined
    ? update.rawOutput
    : current?.rawOutput;
  const normalized = normalizeKiroToolCall({
    kind: readString(update.kind),
    rawInput,
    rawOutput,
    title: rawName,
...
function finalizeTurn(
  ...
  const assistantId = turn.assistantId
    ?? promptId
    ?? `kiro-${scope}-turn-${turn.turnIndex}-assistant`;
...
function extractImageAttachment(
  ...
  return {
    data,
    id: `kiro-${sanitizeId(sessionId)}-turn-${turnIndex}-image-${imageIndex}`,
    mediaType,
    name: `Kiro image ${imageIndex + 1}.${extension}`,
...
export function resolveKiroUpdateMessageId(
  value: unknown,
  role: 'assistant' | 'user',
...
function normalizeUsage(value: unknown): KiroHistoryUsage | undefined {
  const usage = readRecord(value);
  if (!usage) {
    return undefined;
  }
  const normalized: KiroHistoryUsage = {};
...
function parseRecord(line: string): KiroHistoryRecord | null {
  if (!line.trim()) {
    return null;
  }
  try {
