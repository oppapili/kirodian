...
import {
  KIRO_SUBAGENT_CLOSE_TOOL_NAMES,
  KIRO_SUBAGENT_SPAWN_TOOL_NAMES,
  KIRO_SUBAGENT_WAIT_TOOL_NAMES,
} from './kiroLifecycleToolNames';

const KIRO_SUBAGENT_SPAWN_TOOLS: ReadonlySet<string> = new Set(KIRO_SUBAGENT_SPAWN_TOOL_NAMES);
const KIRO_SUBAGENT_WAIT_TOOLS: ReadonlySet<string> = new Set(KIRO_SUBAGENT_WAIT_TOOL_NAMES);
const KIRO_SUBAGENT_CLOSE_TOOLS: ReadonlySet<string> = new Set(KIRO_SUBAGENT_CLOSE_TOOL_NAMES);

interface KiroTaskResult {
  id?: string;
  output?: string;
...
function classifyTaskState(value: unknown): KiroTaskResult['state'] {
...
function parseTaskResults(value: unknown): KiroTaskResult[] {
  ...
  const keyedResults: KiroTaskResult[] = [];
...
function getLifecycleTargetIds(toolCall: ToolCallInfo): string[] {
  ...
  const waitResult = extractKiroWaitResult(toolCall.result, toolCall);
...
export function extractKiroSpawnResult(
...
export function extractKiroWaitResult(
  raw: string | undefined,
  toolCall?: ToolCallInfo,
): ProviderSubagentWaitResult {
...
export function buildKiroSubagentInfo(
  spawnToolCall: ToolCallInfo,
  siblingToolCalls: ToolCallInfo[] = [],
): SubagentInfo {
  const mode = isBackgroundSpawn(spawnToolCall) ? 'async' : 'sync';
  const launch = extractKiroSpawnResult(spawnToolCall.result, spawnToolCall);
  ...
  for (const lifecycleToolCall of siblingToolCalls) {
    ...
    if (KIRO_SUBAGENT_CLOSE_TOOLS.has(name) && lifecycleToolCall.status !== 'running') {
      return {
        ...base,
        asyncStatus: 'error',
    ...
    if (!KIRO_SUBAGENT_WAIT_TOOLS.has(name) || lifecycleToolCall.status === 'running') continue;

    const waitResult = extractKiroWaitResult(lifecycleToolCall.result, lifecycleToolCall);
...
export const kiroSubagentLifecycleAdapter: ProviderSubagentLifecycleAdapter = {
  protocol: 'lifecycle',
  isHiddenTool(name) {
    const normalized = normalizedName(name);
    return KIRO_SUBAGENT_WAIT_TOOLS.has(normalized)
      || KIRO_SUBAGENT_CLOSE_TOOLS.has(normalized);
  },
  ...
  isSpawnTool(name) {
    return KIRO_SUBAGENT_SPAWN_TOOLS.has(normalizedName(name));
  },
  isWaitTool(name) {
    return KIRO_SUBAGENT_WAIT_TOOLS.has(normalizedName(name));
  },
  isCloseTool(name) {
    return KIRO_SUBAGENT_CLOSE_TOOLS.has(normalizedName(name));
  },
  ...
  buildSubagentInfo: buildKiroSubagentInfo,
  extractSpawnResult: extractKiroSpawnResult,
  extractWaitResult: extractKiroWaitResult,
};
