...
const KIRO_TOOL_NAME_MAP: Readonly<Record<string, string>> = {
...
export interface KiroNormalizedToolCall {
...
export interface KiroToolProviderPayload {
  rawInput?: unknown;
  rawName: string;
  rawOutput?: unknown;
}
...
export interface KiroRawToolNameResolution {
  provenance: AcpToolRawNameProvenance;
  rawName: string;
}

export function normalizeKiroToolName(rawName: string): string {
  const normalized = rawName.trim();
  const lookup = normalized.toLowerCase();
  if (KIRO_SUBAGENT_LIFECYCLE_TOOL_NAMES.has(lookup)) {
    return normalized || 'tool';
  }
  return KIRO_TOOL_NAME_MAP[lookup] ?? (normalized || 'tool');
}

export function resolveKiroRawToolName(
  currentRawName: KiroRawToolNameResolution | undefined,
  update: { kind?: string | null; title?: string | null },
): KiroRawToolNameResolution {
  ...
  if (
    normalizedTitle
    && (
      normalizedTitle in KIRO_TOOL_NAME_MAP
      || KIRO_SUBAGENT_LIFECYCLE_TOOL_NAMES.has(normalizedTitle)
    )
  ) {
...
export function normalizeKiroToolCall(value: {
  kind?: string | null;
  rawInput?: unknown;
  rawOutput?: unknown;
  title?: string | null;
}, currentRawName?: KiroRawToolNameResolution): KiroNormalizedToolCall {
  const rawName = resolveKiroRawToolName(currentRawName, value).rawName;
  return {
    input: normalizeToolInput(rawName, value.rawInput),
    name: normalizeKiroToolName(rawName),
...
export function buildKiroToolProviderPayload(value: {
  rawInput?: unknown;
  rawName: string;
  rawOutput?: unknown;
}): KiroToolProviderPayload {
  return {
...
export function normalizeKiroToolUseResult(
  rawName: string,
  input: Record<string, unknown>,
  rawOutput: unknown,
  rawInput?: unknown,
): KiroNormalizedToolUseResult {
  const providerPayload = buildKiroToolProviderPayload({
    rawInput,
    rawName,
    rawOutput,
  });
  const answers = normalizeKiroQuestionAnswers(rawName, input, rawOutput);
  return {
    ...(answers ? { answers } : {}),
    providerPayload,
  };
...
function normalizeKiroQuestionAnswers(
  rawName: string,
  input: Record<string, unknown>,
  rawOutput: unknown,
): AskUserAnswers | undefined {
  if (normalizeKiroToolName(rawName) !== TOOL_ASK_USER_QUESTION || !isRecord(rawOutput)) {
    return undefined;
  }
